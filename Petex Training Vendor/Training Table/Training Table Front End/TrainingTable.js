...
const TrainingTable = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:8012/traning/all')
  ...
  const handleUpdate = () => {
    try {
      fetch(`http://localhost:8012/traning/${editItem.id}`, {
        method: 'put',
        headers: {
  ...
  const handleDelete = (itemId, itemName) => {
    ...
    if (confirmDelete) {
      try {
        fetch(`http://localhost:8012/traning/${itemId}`, {
          method: 'DELETE',
        })
          .then((response) => {
            if (response.ok) {
                  ...
                  {editItem === item ? (
                    ...
                    >
                     <option value="SpaBath">SpaBath</option>
                      <option value="Grooming">Training</option>
                    </select>
                  ) : (
                    item.serviceType
                  )}
